...
# All Ghana districts with coordinates
GHANA_DISTRICTS = {
...
# ── Flood risk logic ─────────────────────────────────────────
def calculate_risk(rainfall, humidity, wind_speed):
    score = 0
...
def get_weather(district_key: str):
    ...
    try:
        ...
        rain = w["rain"]
        risk = calculate_risk(rainfall, humidity, wind)
...
def get_history(district_key: str):
    ...
    c.execute('''SELECT temperature, rainfall, humidity, wind_speed, risk_level, timestamp
        FROM weather_logs WHERE district=? ORDER BY timestamp DESC LIMIT 20''',
        (d["name"],))
    rows = c.fetchall()
    conn.close()
...
def monitor_all():
    ...
    for key, d in GHANA_DISTRICTS.items():
        try:
            ...
            wind = w["wind_speed_10m"]
            risk = calculate_risk(rainfall, humidity, wind)
            save_weather(d["name"], d["region"], w["temperature_2m"], rainfall, humidity, wind, risk)
            save_alert(d["name"], d["region"], risk, rainfall)
            results.append({
                "district": d["name"],
                "region": d["region"],
